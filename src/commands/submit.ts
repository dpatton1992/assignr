...
export function registerSubmitCommand(program: Command, p: ManciplePaths, cwd: string): void {
  program
    ...
    .option(
      "--blocked",
      "Mark as blocked (shortcut for --result blocked). Requires --reason.",
      false,
    )
    ...
    .option(
      ...
      "Acceptance evidence as '<exact criterion text> => <evidence>'. May be repeated.",
      collect,
      [],
    )
  ...
  program
    ...
    .option(
      "--task-status <status>",
      `Final task status to record. Allowed: ${STATUSES.join(", ")}.`,
    )
    .option("--model <model>", "Model that performed the work.")
    .option("--agent <agent>", "Agent harness or tool used.")
    ...
    .option(
      "--input-tokens <count>",
      "Input token count recorded by the agent or harness.",
      parseNumberOption,
    )
    .option(
      "--output-tokens <count>",
      "Output token count recorded by the agent or harness.",
      parseNumberOption,
    )
    .option(
      "--total-tokens <count>",
      "Total token count recorded by the agent or harness.",
      parseNumberOption,
    )
    .option(
      "--cost-usd <amount>",
      "Run cost in USD recorded by the agent or harness.",
      parseNumberOption,
    )
    .option("--command <command>", "Command executed during the run. May be repeated.", collect, [])
    .option(
      "--commands-run <command>",
      "Command executed during the run. May be repeated.",
      collect,
      [],
    )
    .option(
      "--test <command>",
      "Test command or test receipt executed during the run. May be repeated.",
      collect,
      [],
    )
    .option(
    ...
    .option(
      "--file <path>",
      "Changed file path. May be repeated; otherwise git status is used.",
      collect,
      [],
    )
    .option(
    ...
    .action(
      (
        ...
        if (opts.taskStatus && !STATUSES.includes(opts.taskStatus as Status)) {
          console.error(
            `Invalid task status: "${opts.taskStatus}". Allowed: ${STATUSES.join(", ")}`,
          );
          process.exit(1);
        }
