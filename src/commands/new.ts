...
import type { Priority, TaskType } from "../constants.js";
...
const defaultSpecValues: NewTaskSpecValues = {
  ...
  outputsRequired: ["files_changed", "tests_run", "risks", "follow_up_tasks"],
  ...
  if (!result.ok) {
    if (result.code === "duplicate" && result.existingPath) {
      throw new TaskCreationError(
        `Error: task spec already exists at ${result.existingPath.replace(`${cwd}/`, "")}`,
      );
    }
    const prefix =
      result.code === "invalid_type" || result.code === "invalid_priority" ? "" : "Error: ";
...
export async function newInteractiveCommand(
  ...
  const question: PromptQuestion =
    ...
    (async (prompt: string): Promise<string> => {
      // `readline` is created whenever `options.question` is absent, so this
      // branch is only reached when the interface exists.
      if (!readline) {
        throw new Error("Interactive prompt requires a readline interface.");
      }
