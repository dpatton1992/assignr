    ...
    "manciple_prepare_worktree",
    {
      ...
      inputSchema: {
        ...
        use_worktrees: z
          .boolean()
          .optional()
          .describe("Override worktrees.enabled for this preparation."),
      },
    },
    ({ repo, task_id, use_worktrees }) =>
      toolResult(() => {
        ...
        if (!(use_worktrees ?? loadConfig(ctx.cwd).worktrees.enabled)) {
          ...
          if (task.spec.status === "pending")
            setTaskStatus(task_id, "in_progress", ctx.paths.specsTasks);
          return jsonResult({
            mode: "in_place",
            task_id,
  ...
  server.registerTool(
    ...
    ({ repo, task_id }) =>
      toolResult(() => {
        ...
        return record
          ? jsonResult(jsonRecord(record, ctx.cwd))
          : errorResult(`Managed worktree not found: ${task_id}`);
      }),
  );
