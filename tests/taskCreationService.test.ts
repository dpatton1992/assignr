...
function writeTierCopy(id: string, title: string, tier: "completed" | "archived", status: string): void {
...
describe("taskCreationService", () => {
  it("creates a task with a slugified id and writes the YAML spec", () => {
    const result = createTask(baseInput() as never);
  ...
  it("detects a duplicate id in the active tier", () => {
    const first = createTask(baseInput() as never);
    expect(first.ok).toBe(true);

    const second = createTask(baseInput() as never);
  ...
  it("detects a duplicate id in the completed tier", () => {
    writeTierCopy("license-expiration-reminders", "License expiration reminders", "completed", "complete");
  ...
  it("detects a duplicate id in the archived tier", () => {
    writeTierCopy("license-expiration-reminders", "License expiration reminders", "archived", "archived");
  ...
  it("fails when the title cannot generate an id", () => {
    const result = createTask({ ...(baseInput({ title: "!!!" }) as never) });
    expect(result).toEqual({ ok: false, code: "invalid_id", message: "could not generate a valid id from the provided title." });
    expect(existsSync(join(p.tasksActive, "!.yaml"))).toBe(false);
  });

  it("fails when the goal is whitespace-only", () => {
    const result = createTask({ ...(baseInput({ goal: "   " }) as never) });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("fails for an invalid task type", () => {
    const result = createTask({ ...(baseInput({ type: "not-a-type" }) as never) });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("fails for an invalid priority", () => {
    const result = createTask({ ...(baseInput({ priority: "urgent" }) as never) });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("fails when the spec fails schema validation", () => {
    const result = createTask({
      ...(baseInput({ acceptanceCriteria: [] }) as never),
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("never prints, warns, or exits", () => {
    ...
    try {
      const ok = createTask(baseInput() as never);
      expect(ok.ok).toBe(true);
      const duplicate = createTask(baseInput() as never);
      expect(duplicate.ok).toBe(false);
      const invalid = createTask({ ...(baseInput({ title: "!!!" }) as never) });
