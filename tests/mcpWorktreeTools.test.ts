...
function setupRepo(): string {
  ...
  writeFileSync(join(p.tasksActive, "mcp-managed-task.yaml"), `id: mcp-managed-task
...
describe("MCP managed worktree tools", () => {
  it("prepares, reports, releases, and removes a task worktree", async () => {
    ...
    try {
      ...
      const runLog = payload(await client.callTool({
      ...
      const listed = payload(await client.callTool({ name: "manciple_list_worktrees", arguments: {} }));
